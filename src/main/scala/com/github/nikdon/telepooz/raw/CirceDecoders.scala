...
trait CirceDecoders {
  ...
  implicit val asdf:Decoder[String @@ FileId] = Decoder[String].map(_.fileId)
  ...
  implicit val parseModeDecoder: Decoder[ParseMode] = deriveDecoder[String].map(ParseMode.unsafe)
